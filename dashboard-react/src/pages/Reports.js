...
import { FiDownload, FiRefreshCw, FiArrowRight, FiArrowLeft } from 'react-icons/fi';
...
export default function Reports() {
  ...
  const loadPreview = useCallback(async () => {
    ...
    try {
      ...
      const previewData = { total: rows.length, success, denied, rate, checkIn, checkOut, devices, mode2Count, rows };
  ...
  const handleExportAttendance = async () => {
    ...
    try {
      const rows = preview.rows
        ...
        'Chiều di chuyển': r.direction === 'OUT' ? 'Ra' : r.direction === 'IN' ? 'Vào' : '-',
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 20 }}>
              <div style={{
                ...
                <div style={{ fontSize: 28, color: '#0284c7' }}><FiArrowRight /></div>
                <div>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600 }}>Vào (IN)</div>
                  <div style={{ fontSize: 22, color: '#0284c7', fontWeight: 700 }}>{preview.checkIn || 0}</div>
                </div>
              </div>
              <div style={{
                background: '#fecaca',
                ...
                <div style={{ fontSize: 28, color: '#dc2626' }}><FiArrowLeft /></div>
                <div>
                  <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600 }}>Ra (OUT)</div>
                  <div style={{ fontSize: 22, color: '#dc2626', fontWeight: 700 }}>{preview.checkOut || 0}</div>
                </div>
              </div>
            </div>
            <div>

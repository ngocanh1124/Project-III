...
import { FiArrowLeft, FiUnlock, FiRefreshCw, FiDownload, FiArrowRight } from 'react-icons/fi';
...
export default function DeviceDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [statsRes, deviceRes, histRes, specialRes, violationsRes] = await Promise.allSettled([
        deviceService.getStats(deviceCode),
        deviceService.getByDeviceCode(deviceCode), // Fetch complete device info with direction
      ...
      if (deviceRes.status === 'fulfilled') {
        // Extract the first device from search results that matches deviceCode
        const deviceList = deviceRes.value.data?.data?.content || [];
        const matchedDevice = deviceList.find(d => d.deviceCode === deviceCode);
        if (matchedDevice) {
          setDevice(matchedDevice); // This now includes direction field
        }
      }
  ...
  useAttendanceSocket((log) => {
    ...
    const isWarning = log.alertType === 'MISSING_IN_AUTO_OPEN';
    if (isUnlock) {
      setSpecialCases((prev) => [log, ...prev]);
    } else if (isOutsideHours) {
      setViolationLogs((prev) => [log, ...prev]);
    } else if (isWarning) {
      // Thêm warning logs vào logs array
      setLogs((prev) => [log, ...prev.slice(0, 14)]);
      setStats((s) => s ? {
        ...s,
        todayAttempts: (s.todayAttempts || 0) + 1,
        todaySuccess: (s.todaySuccess || 0) + 1,  // Warning cũng là success (door opened)
        monthAttempts: (s.monthAttempts || 0) + 1,
        monthSuccess: (s.monthSuccess || 0) + 1,
      } : s);
    } else {
      setLogs((prev) => [log, ...prev.slice(0, 14)]);
      setStats((s) => s ? {
  ...
  const successLogs = logs.filter(r => r.accessGranted === true && r.alertType !== 'MISSING_IN_AUTO_OPEN');
  const warningLogs = logs.filter(r => r.alertType === 'MISSING_IN_AUTO_OPEN' || r.alertType === 'ANTI_PASSBACK_IN');
  const failedLogs = logs.filter(r => r.accessGranted !== true && r.alertType !== 'OUTSIDE_HOURS' && r.alertType !== 'ANTI_PASSBACK_IN');
  const tableLogs = tab === 'success' ? successLogs : tab === 'warnings' ? warningLogs : tab === 'failed' ? failedLogs : tab === 'special' ? specialCases : violationLogs;
            ...
            <span style={{ fontSize: 13, color: '#6b7280' }}>
              ...
              {device?.direction && (
                <span style={{ 
                  ...
                  alignItems: 'center', 
                  gap: 4,
                  background: device.direction === 'OUT' ? '#fecaca' : '#dbeafe',
                  color: device.direction === 'OUT' ? '#dc2626' : '#0284c7',
                  padding: '2px 8px',
                  borderRadius: 4,
                  fontWeight: 600,
                  fontSize: 11
                }}>
                  {device.direction === 'OUT' ? <FiArrowLeft size={12} /> : <FiArrowRight size={12} />}
                  {device.direction === 'OUT' ? 'Ra (OUT)' : 'Vào (IN)'}
                </span>
              )}            
            </span>
          </div>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            ...
            <button
              className={`btn btn-sm ${tab === 'warnings' ? '' : 'btn-secondary'}`}
              style={tab === 'warnings' ? { background: '#f59e0b', color: 'white', border: 'none' } : {}}
              onClick={() => setTab('warnings')}
            >
              Cảnh báo ({warningLogs.length})
            </button>
            <button
          ...
          {(tab === 'success' || tab === 'warnings' || tab === 'failed') && (
        ...
        {loading ? (
          ...
          <>
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    {tab === 'special' ? <th>Lý do</th> : tab === 'warnings' ? <th>Ghi chú</th> : <th>Độ khớp</th>}
                    <th>Ảnh</th>
                  </tr>
                </thead>
                <tbody>
                  {tableLogs.length === 0 ? (
                    ...
                    groupLogsByDate(tableLogs).flatMap(([date, logsForDate]) => [
                      ...
                      ...logsForDate.map((r) => {
                        ...
                        const displayDirection = r.direction || device?.direction || 'IN';
                        const directionBg = displayDirection === 'OUT' ? { bg: '#fecaca', color: '#dc2626' } : { bg: '#dbeafe', color: '#0284c7' };
                        return (
                          <tr key={r.id} style={tab === 'warnings' ? { background: '#fffbeb' } : {}}>
                            ...
                            <td>
                              <span className="badge" style={{ background: directionBg.bg, color: directionBg.color, fontSize: 11, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                {displayDirection === 'OUT' ? <FiArrowLeft size={12} /> : <FiArrowRight size={12} />}
                                {displayDirection === 'OUT' ? 'Ra' : 'Vào'}
                              </span>
                            </td>
            ...
            {}
            {(tab === 'success' || tab === 'warnings' || tab === 'failed') && totalPages > 1 && (

...
static inline void relayWrite(int pin, bool turnOn) {
    bool physLevel;
#if RELAY_ACTIVE_LOW
    physLevel = turnOn ? LOW : HIGH;
#else
    physLevel = turnOn ? HIGH : LOW;
#endif
    digitalWrite(pin, physLevel);
    Serial.printf("   [RELAY_PIN %d] -> %s (%s)\n", pin, (physLevel == HIGH ? "HIGH" : "LOW"), (turnOn ? "ACTIVE" : "INACTIVE"));
}
...
void openDoor(int doorIdx, int durationMs, const char* openerName, const char* source) {
    ...
    Serial.printf("[RELAY%d/%s] >>> MO CUA | GPIO %d | %d ms | %s | nguon: %s\n",
                  doorIdx + 1, DOORS[doorIdx].deviceCode, relayPin, durationMs, r.opener, r.source);
}
...
void publishLog(const char* action, int doorIdx, const char* name,
    ...
    // Thêm trạng thái relay vào log để backend hiển thị
    doc["relay1"] = relayStates[0].active ? "OPEN" : "CLOSED";
    if (NUM_DOORS > 1) {
        doc["relay2"] = relayStates[1].active ? "OPEN" : "CLOSED";
    }
...
void publishStatus() {
    ...
    StaticJsonDocument<512> doc;
    doc["type"]       = "STATUS";
    doc["deviceCode"] = DOORS[0].deviceCode; // Backend expects deviceCode here too
    doc["online"]     = true;
    doc["bleConn"]    = bleConnected;
    doc["freeHeap"]   = ESP.getFreeHeap();
    doc["uptime_ms"]  = millis();
    
    // Thêm relay1, relay2 trực tiếp vào root để backend dễ đọc
    doc["relay1"] = relayStates[0].active ? "OPEN" : "CLOSED";
    if (NUM_DOORS > 1) {
        doc["relay2"] = relayStates[1].active ? "OPEN" : "CLOSED";
    }
...
void handleCommand(const char* jsonStr, const char* source, int forceDoorIdx = -1) {
    ...
    // Tăng giới hạn parse JSON lên 1024
    StaticJsonDocument<1024> doc;
    DeserializationError err = deserializeJson(doc, jsonStr);
    if (err) {
        Serial.printf("[CMD/%s] JSON loi (%s): %s\n", source, err.c_str(), jsonStr);
        return;
    }
    ...
    // Mặc định relay = true cho OPEN_DOOR nếu server gửi action này
    bool relay = true;
    if (doc.containsKey("relay")) {
        relay = doc["relay"].as<bool>();
    }
    ...
    if (strcmp(action, "OPEN_DOOR") == 0) {
        Serial.printf("[DOOR] OPEN_DOOR for %s | channel=%d | relayVal=%d\n", name, doorIdx + 1, relay);
        if (!relay) {
            Serial.printf("[CMD/%s] OPEN_DOOR bi bo qua vi relay=false\n", source);
            publishLog("OPEN_DOOR_IGNORED", doorIdx, name, false, "relay=false", source);
            return;
        }
...
void onMqttMessage(char* topic, byte* payload, unsigned int length) {
    // Tăng buffer MQTT lên 1024 để nhận JSON dài
    char buf[1024];

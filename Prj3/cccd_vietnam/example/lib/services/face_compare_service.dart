...
class FaceCompareService {
  static const double _matchThreshold   = 0.55;  // Ha xuong 0.55 (phu hop chip image)
  static const double _backupThreshold  = 0.45;  // Chi reject L1 khi score rat thap; server quyet dinh cuoi
  ...
  static Future<Map<String, dynamic>?> compareFaces({
    ...
    final String springBaseUrl = ConfigService.baseUrl;
    ...
    if (l1DetectedFace && l1Score < _backupThreshold) {
      // Đối với MỞ CỬA TỪ XA: Cho phép skip Layer 1 reject để Admin/Server quyết định
      if (remoteUnlock) {
        print('FaceCompareService: L1 score low ($l1Score) but remoteUnlock=true -> bypassing local reject');
        l1Passed = false; // Báo cho server biết L1 không pass nhưng vẫn gửi payload
      } else {

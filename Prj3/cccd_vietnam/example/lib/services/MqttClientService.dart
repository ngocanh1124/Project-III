...
class MqttClientService {
  ...
  static Future<void> initialize(Function(Map<String, dynamic>) onCommandReceived) async {
    ...
    _client!.updates!.listen((List<MqttReceivedMessage<MqttMessage>> c) async {
      ...
      try {
        ...
        if (data['action'] == 'OPEN_DOOR' || data['action'] == 'START_AUTH_FLOW') {
          // Forward cả OPEN_DOOR và START_AUTH_FLOW tới callback đã đăng ký
          _savedCallback?.call(data); // Always use latest registered callback
        }
        else if (data['action'] == 'SYNC_DATA') {
  ...
  static const String TOPIC_SERVER_IP = 'cccd/server/ip';
  ...
  static void _onConnected() {
    ...
    // REMOVED: cccd/server/ip subscription — HiveMQ credential only allows cccd/devices/#
    // Subscribing to cccd/server/ip causes broker to send DISCONNECT immediately

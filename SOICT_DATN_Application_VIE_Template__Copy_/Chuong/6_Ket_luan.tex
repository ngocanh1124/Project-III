...
\begin{itemize}
    \item \textbf{Ứng dụng di động Flutter (Trạm đầu cuối):} Thực hiện nghiệp vụ bóc tách NFC theo giao thức BAC/PACE và đối soát lớp 1 bằng MobileFaceNet chạy offline trực tiếp trên thiết bị thông qua ONNX Runtime (\texttt{onnxruntime.dart}) mà không phụ thuộc vào mạng Internet.
    \item \textbf{Lõi máy chủ Backend Spring Boot:} Điều phối toàn bộ luồng xác thực lớp 2 nâng cao, quản lý ma trận phân quyền, lưu trữ cơ sở dữ liệu và xử lý các logic nghiệp vụ chấm công hành chính. Đặc biệt, hệ thống đã hiện thực hóa thành công logic kiểm soát Anti-passback và cơ chế tự phục hồi trạng thái (Self-healing) giúp bảo đảm quy trình ra vào đúng trình tự.
    \item \textbf{Dịch vụ tính toán Python Flask:} Tiếp nhận request payload, bóc tách chuỗi hình ảnh và xử lý các tác vụ so sánh độ tương đồng hình học không gian Cosine bằng mô hình học sâu chuyên sâu ArcFace.
    \item \textbf{Bo mạch vi điều khiển ESP32:} Kết nối kéo dài qua giao thức TCP ổn định, đóng vai trò cơ cấu chấp hành nhận lệnh từ máy chủ để đóng ngắt rơ-le khóa cửa vật lý một cách an toàn.
    \item \textbf{Giao diện Web React:} Cung cấp hệ thống quản trị, cấu hình ma trận phân quyền, quản lý hồ sơ nhân sự, xuất báo cáo Excel và hiển thị DashBoard giám sát dòng người ra vào theo thời gian thực với mã màu phân biệt chiều di chuyển.
\end{itemize}

Về mặt học thuật và kỹ thuật, đồ án đã vận dụng thành công nhiều giải pháp công nghệ hiện đại trong một cấu trúc kiến trúc phân tán mạch lạc, tối ưu: ứng dụng mô hình học sâu InsightFace đạt độ chính xác cao; giao thức bảo mật ICAO Doc 9303 \cite{icao9303} cho phân hệ đọc chip thẻ vật lý; giao thức mạng nhẹ MQTT over SSL/TLS qua hạ tầng đám mây HiveMQ Cloud \cite{hivemq} cổng 8883 cho phân hệ điều khiển phần cứng; giải pháp mã hóa phi trạng thái JWT (\texttt{SignatureAlgorithm.HS256}) phối hợp bộ lọc Spring Security và kiểm soát đặc quyền \texttt{@PreAuthorize} (RBAC năm cấp bao gồm \texttt{SUPER\_ADMIN}, \texttt{ADMIN}, \texttt{HR\_MANAGER}, \texttt{OPERATOR}, \texttt{VIEWER}) cho hành lang bảo mật REST API; và đường ống song công WebSocket STOMP \cite{rfc6455} cho phân hệ Web Dashboard. Đồ án cũng đã chứng minh tính hiệu quả của mô hình Máy trạng thái tự phục hồi (Self-healing State Machine) trong việc duy trì tính toàn vẹn dữ liệu điểm danh.

kết quả thực nghiệm khẳng định hệ thống đạt được sự cân bằng tối ưu giữa ba tiêu chí: độ chính xác sinh trắc học đạt mức tin cậy cao với ngưỡng quyết định $0.55$ được cấu hình an ninh, tốc độ xử lý phản hồi thời gian thực ấn tượng (đạt trung bình 2.8 - 3.8 giây cho toàn bộ chu trình đầu cuối) và nâng cao hiệu quả quản trị khi giảm thiểu 88\% thời gian đối soát thủ công nhờ cơ chế tự động hóa kiểm soát Anti-passback và cấu trúc điều khiển MQTT dải rộng (Direct & Broadcast).
...
\begin{enumerate}
    \item \textbf{Tư duy thiết kế hệ thống bất đồng bộ:} Việc xử lý luồng truyền thông đa thiết bị đòi hỏi phải từ bỏ tư duy đồng bộ truyền thống. Giải pháp tối ưu hóa hồ chứa luồng (Thread Pool) phối hợp cấu trúc Async Executor của Spring Boot và hai đường ống truyền dữ liệu song song (MQTT/WebSocket) là chìa khóa để triệt tiêu hiện tượng nghẽn mạch cục bộ (Network Bottleneck).
    \item \textbf{Tầm quan trọng của thiết kế kiến trúc an ninh đa lớp:} An ninh hệ thống không thể chỉ dựa vào một bộ lọc duy nhất. Việc bao bọc REST API bằng hành lang bảo mật bám sát tiêu chuẩn OWASP Top 10 (tham số hóa JPA chống SQL Injection, ký số mã hóa JWT phi trạng thái, thiết lập nghiêm ngặt CORS) bảo chứng cho năng lực vận hành tin cậy của sản phẩm khi đưa vào môi trường Production thực tế.
...
\begin{itemize}
    \item \textbf{Hiện thực hóa toàn diện thuật toán Passive Authentication:} Nghiên cứu cài đặt quy trình bóc tách và xác chuẩn phân vùng tệp tin SOD theo đúng đặc tả kỹ thuật ICAO Doc 9303 \cite{icao9303}. Phân hệ này sẽ nạp danh sách chứng chỉ số công khai Root CA của Bộ Công an Việt Nam để kiểm tra chữ ký điện tử, giúp máy chủ phát hiện và chặn đứng hoàn toàn các nguy cơ sử dụng chip thẻ giả mạo hoặc tệp tin mã nhị phân đã bị sửa đổi.
    ...
    \item \textbf{Tinh chỉnh mô hình AI trên dữ liệu đặc trưng nội địa:} Tổ chức thu thập tập mẫu khuôn mặt người Việt Nam và thực hiện kỹ thuật tinh chỉnh mô hình (Fine-tuning) trên mô hình nền ArcFace \cite{deng2019arcface} nhằm tối ưu hóa năng lực bóc tách vector đặc trưng cho các đặc điểm hình học khuôn mặt đặc thù của người nội địa, đẩy chỉ số TPR lên mức trên 97\%.
\end{itemize}

\subsection{Nghiên cứu các hướng mở rộng, nâng cấp hệ thống chuyên sâu}
\begin{itemize}
    \item \textbf{Nâng cấp hạ tầng triển khai lên kiến trúc phân tán quy mô lớn:} Để chuẩn bị cho kịch bản đưa vào vận hành thực tế tại các tập đoàn có hàng vạn nhân sự và hàng trăm trạm cửa vật lý chạy đồng thời, toàn bộ hạ tầng cần được Container hóa bằng Docker, điều phối tự động bằng Kubernetes hỗ trợ cơ chế tự động co giãn (Horizontal Pod Autoscaling). Đồng thời, dịch vụ tính toán AI cần được dịch chuyển sang cụm máy chủ chuyên dụng GPU Cluster để bảo đảm độ trễ tính toán không bị phình to khi số lượng yêu cầu xác thực tăng cao đột biến.

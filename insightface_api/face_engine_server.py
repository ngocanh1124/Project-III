    ...
    try:
        ...
        try:
            def decode_base64_to_cv2(b64_string):
                ...
                # Try decoding with CV2 first
                img_array = np.frombuffer(img_data, dtype=np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                
                # If CV2 fails (e.g. JPEG2000 or corrupted header), try PIL
                if img is None:
                    try:
                        ...
                        logger.info("Decoded image using PIL (fallback for JP2/other)")
                    except Exception as e:
                        logger.error(f"PIL fallback decoding failed: {e}")
                
                return img

            face1_image = decode_base64_to_cv2(data['face1_base64'])
            face2_image = decode_base64_to_cv2(data['face2_base64'])
            
            if face1_image is None or face2_image is None:
                logger.error(f"Image decode failed: face1={face1_image is None}, face2={face2_image is None}")
                return jsonify({
                    'success': False,
                    'error': 'Invalid image data (decode failed). Check if images are valid JPEGs or PNGs.',
                    'timestamp': datetime.now().isoformat()
                }), 400
        except Exception as e:
        ...
        # Face analysis with fallback for small images
        try:
            faces1 = app.face_engine.get(face1_image)
            if not faces1 and face1_image is not None:
                # Retry for small chip images
                logger.info("Retrying face detection for face1 with upscaling...")
                h, w = face1_image.shape[:2]
                if h < 200 or w < 200:
                    scaled = cv2.resize(face1_image, (0,0), fx=2.0, fy=2.0)
                    faces1 = app.face_engine.get(scaled)

            faces2 = app.face_engine.get(face2_image)
            if not faces2 and face2_image is not None:
                logger.info("Retrying face detection for face2 (chip image) with enhancement...")
                h, w = face2_image.shape[:2]
                
                # Chiến lược 1: Upscale
                scaled = cv2.resize(face2_image, (0,0), fx=2.0, fy=2.0)
                faces2 = app.face_engine.get(scaled)
                ...
                if not faces2:
                    ...
                    faces2 = app.face_engine.get(enhanced)
            
            if not faces1 or not faces2:
                logger.warning(f"Detection failed: faces1={len(faces1)}, faces2={len(faces2)}")
                return jsonify({
                    'success': False,
        ...
        try:
            ...
            # Lấy ngưỡng (Dựa trên thực nghiệm ArcFace / InsightFace)
            # Nếu request có ngưỡng riêng thì dùng, không thì mặc định 0.72
    ...
    try:
        ...
        try:
            ...
            if not faces:
                return jsonify({
                    'success': False,
                    'error': 'Không tìm thấy khuôn mặt trong ảnh selfie',
                    'timestamp': datetime.now().isoformat()
                }), 400

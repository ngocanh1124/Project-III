...
def compare_with_insightface(face1_b64, face2_b64, threshold=0.55):
    ...
    try:
        ...
        # Decode images with labels for better error reporting
        img1 = base64_to_cv2(face1_b64, label="face1 (selfie)")
        if img1 is None:
            return {
                "success": False, 
                "error": "Ảnh selfie không hợp lệ hoặc không thể giải mã.",
                "face1_decoded": False,
                "face2_decoded": None
            }
            
        img2 = base64_to_cv2(face2_b64, label="face2 (chip/stored)")
        if img2 is None:
            return {
                "success": False, 
                "error": "Ảnh chip CCCD không hợp lệ hoặc không thể giải mã.",
                "face1_decoded": True,
                "face2_decoded": False
            }
        
        # Face analysis with fallback for small/blurry images
        faces1 = g_engine['instance'].get(img1)
        if not faces1 and img1 is not None:
            logger.info("Retrying face detection for face1 with upscaling...")
            h, w = img1.shape[:2]
            if h < 200 or w < 200:
                scaled = cv2.resize(img1, (0,0), fx=2.0, fy=2.0)
                faces1 = g_engine['instance'].get(scaled)

        faces2 = g_engine['instance'].get(img2)
        if not faces2 and img2 is not None:
            logger.info("Retrying face detection for face2 (chip image) with enhancement...")
            # Chiến lược 1: Upscale
            scaled2 = cv2.resize(img2, (0,0), fx=2.0, fy=2.0)
            faces2 = g_engine['instance'].get(scaled2)
            
            # Chiến lược 2: Cân bằng sáng (CLAHE) nếu vẫn không tìm thấy
            if not faces2:
                logger.info("Retrying face detection for face2 with CLAHE...")
                lab = cv2.cvtColor(scaled2, cv2.COLOR_BGR2LAB)
                l, a, b = cv2.split(lab)
                clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
                cl = clahe.apply(l)
                limg = cv2.merge((cl,a,b))
                enhanced = cv2.cvtColor(limg, cv2.COLOR_LAB2BGR)
                faces2 = g_engine['instance'].get(enhanced)
        
        if len(faces1) == 0 or len(faces2) == 0:
            logger.warning(f"InsightFace: no face detected (faces1={len(faces1)}, faces2={len(faces2)})")
            return {
                "success": False,
        ...
        matched = bool(similarity >= float(threshold))
...
def compare_with_facenet(face1_b64, face2_b64, threshold=0.55):
    ...
    try:
        ...
        if img1 is None:
            return {"success": False, "error": "Invalid image: face1_base64 cannot be decoded"}
            
        img2 = base64_to_cv2(face2_b64)
        if img2 is None:
            return {"success": False, "error": "Invalid image: face2_base64 cannot be decoded"}
        
        # Face analysis with enhancement
        faces1 = g_engine['mtcnn'](img1)
        if faces1 is None and img1 is not None:
            logger.info("FaceNet: retrying face1 with upscaling...")
            scaled1 = cv2.resize(img1, (0,0), fx=2.0, fy=2.0)
            faces1 = g_engine['mtcnn'](scaled1)

        faces2 = g_engine['mtcnn'](img2)
        if faces2 is None and img2 is not None:
            logger.info("FaceNet: retrying face2 with enhancement...")
            scaled2 = cv2.resize(img2, (0,0), fx=2.0, fy=2.0)
            faces2 = g_engine['mtcnn'](scaled2)
            if faces2 is None:
                lab = cv2.cvtColor(scaled2, cv2.COLOR_BGR2LAB)
                l, a, b = cv2.split(lab)
                clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
                ...
                faces2 = g_engine['mtcnn'](enhanced)
        
        if faces1 is None or faces2 is None or len(faces1) == 0 or len(faces2) == 0:
            logger.warning("FaceNet: no face detected in one or both images")
            return {
                "success": False,
                "error": "No face detected in one or both images",
                "face1_detected": faces1 is not None and len(faces1) > 0,
                "face2_detected": faces2 is not None and len(faces2) > 0,
                "method": "FaceNet"
            }
...
def base64_to_cv2(image_base64, label="image"):
    ...
    try:
        ...
        if not image_base64 or not isinstance(image_base64, str):
            logger.error(f"base64_to_cv2 ({label}): Input is not a string or empty")
            return None

        # Pre-process: strip data URL prefix if present
        ...
        logger.info(f"base64_to_cv2 ({label}): Decoding base64, len={len(image_base64)}")
        try:
            image_bytes = base64.b64decode(image_base64)
        except Exception as b64e:
            logger.error(f"base64_to_cv2 ({label}): base64 decode failed: {b64e}")
            return None

        try:
            image = Image.open(BytesIO(image_bytes))
        except Exception as img_e:
            logger.error(f"base64_to_cv2 ({label}): PIL open failed: {img_e}. Bytes len={len(image_bytes)}")
            # Try OpenCV decode as fallback if PIL fails (maybe for some formats like JPEG2000 on some systems)
            try:
                nparr = np.frombuffer(image_bytes, np.uint8)
                image_array = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
                if image_array is not None:
                    logger.info(f"base64_to_cv2 ({label}): Recovered image using cv2.imdecode")
                    return image_array
            except Exception as cv_e:
                logger.error(f"base64_to_cv2 ({label}): cv2.imdecode also failed: {cv_e}")
            return None
        ...
        logger.error(f"base64_to_cv2 ({label}): Fatal error: {e}")
        return None
        if image.mode != "RGB":
            image = image.convert("RGB")
...
def face_compare():
    ...
    try:
        ...
        logger.info(f"Face comparison request - Engine: {g_engine_type}, Threshold: {threshold}")
        
        if g_engine_type == 'insightface':
            result = compare_with_insightface(face1_b64, face2_b64, threshold)
        elif g_engine_type == 'facenet':
            result = compare_with_facenet(face1_b64, face2_b64, threshold)
        else:
